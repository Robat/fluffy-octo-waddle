...
class PTestObserver
{
    ...
    public function creating(PTest $model)
    {
    ...
    public function created(PTest $model)
    {
        $PTestCount = $model->select('id', 'frequency_id', 'name', 'sort', 'numbering')->where('frequency_id', admin()->company->frequency()->id)->where('status', 1)->orderBy('sort', 'ASC')->get()->toArray();


        if (count($PTestCount) > 0) {
            for ($i = 0; $i < count($PTestCount); $i++) {
                if ($i == 0) {
                    PDiff::updateOrCreate(
                        ['slug' => $PTestCount[0]['frequency_id'] . '_' . '0'],
                        [
                    ...
                    PDiff::updateOrCreate(
                        ['slug' => $PTestCount[0]['frequency_id'] . '_' . $i],
                        [
            ...
            PDiff::updateOrCreate(
                ['slug' => $PTestCount[0]['frequency_id'] . '_' . count($PTestCount)],
                [
        ...
        $a_point_p = intval(count($ATestCount) / 2) * 2 + 1;
        ...
        $p_point = intval(count($PTestCount) / 2) * 2 + 1;
        ...
        $pdiffs = PDiff::select('id', 'frequency_id', 'name', 'sort', 'numbering')->where('frequency_id', admin()->company->frequency()->id)->where('status', 1)->orderBy('sort', 'ASC')->get()->toArray();
