...
class CompanyADiff extends Model
{
    use HasFactory;
    protected $fillable = ['name', 'sort', 'numbering', 'status', 'bonus_calculation_id', 'frequency_id'];
    protected $guarded = [];

    protected static function register(Builder $builder)
    {
    ...
    {
        return $this->belongsToMany(CompanyCDiff::class)
            ->withPivot('score', 'c_sort', 'a_sort')
            ->withTimestamps()
            ->wherePivot('frequency_id', admin()->company->frequency()->id);
    }
